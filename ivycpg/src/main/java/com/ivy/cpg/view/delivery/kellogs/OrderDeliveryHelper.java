...
public class OrderDeliveryHelper {
    ...
    private void loadSalesReturnData(String orderId, DBUtil db, Context mContext) {
        ...
        try {
            ...
            String sb = "select SI.productid,SI.Condition,SI.Pqty,SI.Cqty,SI.outerqty,SH.uid" +
                    ...
                    "where SH.Retailerid=" + StringUtils.getStringQueryParam(businessModel.getRetailerMasterBO().getRetailerID()) + " and SH.RefModuleTId = '" + orderId + "' and SH.distributorid=" + businessModel.getRetailerMasterBO().getDistributorId();
    ...
    private void loadSalesReplacementData(DBUtil db, String uid) {
        String sb = "select pid,batchid,uomid,qty from SalesReturnReplacementDetails " +
                ...
                " and uid = " + StringUtils.getStringQueryParam(uid);
        Cursor c = db.selectSQL(sb);
        if (c.getCount() > 0) {
            while (c.moveToNext()) {
        ...
        try {
            ...
            Cursor c = db
                    .selectSQL("select schemeid,FreeProductID,FreeQty,UomID,pm.PName from schemeFreeProductDetail " +
                            ...
                            + "where orderID="
                            + StringUtils.getStringQueryParam(id)
                            + " order by schemeid");
            if (c.getCount() > 0) {
                while (c.moveToNext()) {
    ...
    void downloadOrderDeliveryAmountDetail(Context context, String id) {
        try {
            ...
            Cursor orderHeader = db.selectSQL("select OrderValue,discount from OrderHeader " +
                    "where orderid=" + StringUtils.getStringQueryParam(id));
...
//                                + (productBo.getOrderedOuterQty() * productBo.getOsrp());
                        ...
                        String sb = (StringUtils.getStringQueryParam(invoiceId) + ",") +
                                StringUtils.getStringQueryParam(productBo.getProductID()) + "," +
                                totalqty + "," + productBo.getSrp() + "," +
                                StringUtils.getStringQueryParam(productBo.getOU()) + "," +
                                ...
                                "," + productBo.getWeight() +
                                "," + StringUtils.getStringQueryParam(productBo.getHsnCode()) +
                                "," + line_total_price;
                ...
                if (linesPerCall > 0) {
                    ...
                    String invoiceHeaderQry = "Insert into InvoiceMaster (invoiceno,invoicedate,beatid,retailerId,invNetamount,discountedAmount," +
                            ...
                            " select " + invoiceId + "," + StringUtils.getStringQueryParam(DateTimeUtils.now(DateTimeUtils.DATE_GLOBAL)) + ",RouteId,retailerid," +
                            ...
                            "sid,SParentID,stype,imgName,PrintFilePath,timestampid,RemarksType,RField1,RField2,RField3,'N'," + StringUtils.getStringQueryParam(getOrderDeliveryTaxAmount()) + " , " + salesReturned + " , " + businessModel.getRetailerMasterBO().getCreditDays() + " , " + 0 +
                            "," + totalAmount + ",0," + StringUtils.getStringQueryParam(businessModel.getAppDataProvider().getRetailMaster().getRidSF())  +
                            "," + businessModel.getAppDataProvider().getUniqueId() + " from OrderHeader where OrderId = " + StringUtils.getStringQueryParam(orderId);
                ...
                String invoiceHeaderQry = "Insert into InvoiceMaster (invoiceno,invoicedate,beatid,retailerId,invNetamount," +
                        ...
                        " select " + invoiceId + "," + StringUtils.getStringQueryParam(DateTimeUtils.now(DateTimeUtils.DATE_GLOBAL)) + ",RouteId,retailerid," + StringUtils.getStringQueryParam(businessModel.formatBasedOnCurrency(totalOrderValue + SDUtil.convertToDouble(getOrderDeliveryTaxAmount()))) +
                        ",orderid,imagename,discount," + StringUtils.getStringQueryParam(getOrderDeliveryTotalValue()) + ",latitude,longitude,ReturnValue,discount_type,LinesPerCall,totalWeight,SalesType," +
                        "sid,SParentID,stype,imgName,PrintFilePath,timestampid,RemarksType,RField1,RField2,RField3,'N'," +
                        StringUtils.getStringQueryParam(getOrderDeliveryTaxAmount()) + " , " + salesReturned + " , " + businessModel.getRetailerMasterBO().getCreditDays() + " , " + 0 + " , " + StringUtils.getStringQueryParam(businessModel.formatBasedOnCurrency(discountedAmount)) +
                        "," + totalAmount + ",0," + StringUtils.getStringQueryParam(businessModel.getAppDataProvider().getRetailMaster().getRidSF())  +
                        "," + businessModel.getAppDataProvider().getUniqueId() + " from OrderHeader where OrderId = " + StringUtils.getStringQueryParam(orderId);
                ...
                String invoiceDetailQry = "Insert into InvoiceDetails " +
                        ...
                        " dOuomQty,dOuomid,BatchId,CasePrice,OuterPrice,PcsUOMId,OrderType,HsnCode,RField1,totalamount,PriceOffValue,PriceOffId,weight," + StringUtils.getStringQueryParam(invoiceId) +
                        " ,NetAmount from OrderDetail where OrderId = " + StringUtils.getStringQueryParam(orderId);
                ...
                String invoiceDiscountQry = "Insert into InvoiceDiscountDetail (OrderId,Pid,TypeId,Value,Percentage,ApplyLevelId," +
                        ...
                        " RetailerId,DiscountId,isCompanyGiven," + invoiceId + " from OrderDiscountDetail where OrderId = " + StringUtils.getStringQueryParam(orderId);

                db.executeQ(invoiceDiscountQry);

                db.updateSQL("update SchemeFreeProductDetail set upload='N',InvoiceID = " + StringUtils.getStringQueryParam(invoiceId) + " where orderId = " + StringUtils.getStringQueryParam(orderId));
                ...
                String invoiceTaxDetail = "Insert into InvoiceTaxDetails (orderId,pid,taxRate,taxType,taxValue,retailerid,groupid,IsFreeProduct,invoiceid,applyLevelId) " +
                        " select orderId,pid,taxRate,taxType,taxValue,retailerid,groupid,IsFreeProduct," + invoiceId + ",applyLevelId from OrderTaxDetails where OrderId = " + StringUtils.getStringQueryParam(orderId);

                db.executeQ(invoiceTaxDetail);

                for (ProductMasterBO productBo : getOrderedProductMasterBOS()) {
                    Cursor c = db.selectSQL("select ifnull(sum(taxValue),0) from OrderTaxDetails where OrderID=" + StringUtils.getStringQueryParam(orderId) + " and pid = '" + productBo.getProductID() + "'");
                    if (c != null) {
                        if (c.moveToNext()) {
                            db.updateSQL("Update InvoiceDetails set TaxAmount = '" + c.getString(0) + "' where ProductID = '" +
                                    productBo.getProductID() + "' and invoiceID = " + StringUtils.getStringQueryParam(invoiceId));
                        }
                        c.close();
                    }
                }
            ...
            db.updateSQL("update OrderHeader set is_vansales=1,invoicestatus = 1,totalamount =" + totalAmount + " where orderId = " + StringUtils.getStringQueryParam(orderId));
            db.updateSQL("update SalesReturnHeader set IFLAG=0,upload='N',invoiceid = " + StringUtils.getStringQueryParam(invoiceId) + " where RefModuleTId = " + StringUtils.getStringQueryParam(orderId));

            String uid = "";
            Cursor c = db.selectSQL("select uid from SalesReturnHeader where RefModuleTId = " + StringUtils.getStringQueryParam(orderId));
            if (c.getCount() > 0 && c.moveToNext()) {
                uid = c.getString(0);
                db.updateSQL("update SalesReturnDetails set upload='N', invoiceno = " + StringUtils.getStringQueryParam(invoiceId) + " where uid = " + StringUtils.getStringQueryParam(uid));
                c.close();
                db.updateSQL("update SalesReturnReplacementDetails set upload='N' where uid = " + StringUtils.getStringQueryParam(uid));
            }
            ...
            if (businessModel.configurationMasterHelper.IS_CREDIT_NOTE_CREATION || businessModel.configurationMasterHelper.TAX_SHOW_INVOICE) {
                ...
                salesReturnHelper.setSalesReturnID(StringUtils.getStringQueryParam(uid));
                salesReturnHelper.saveSalesReturnTaxAndCreditNoteDetail(context,db, StringUtils.getStringQueryParam(uid), "ORDER", businessModel.retailerMasterBO.getRpTypeCode(),true);
            }
            ...
            String values = StringUtils.getStringQueryParam(orderId) + "," + StringUtils.getStringQueryParam(referenceId);
    ...
    private void insertProductLevelTax(String orderId, DBUtil db,
        ...
        values.append(StringUtils.getStringQueryParam(orderId)).append(",")
        ...
        values.append(",")
                ...
                .append(",0")
                .append(",")
                .append(StringUtils.getStringQueryParam(invoiceId))
                .append(",")
    ...
    private int isCreditNoteCreated(Context mContext, String orderId) {
        ...
        try {
            ...
            db.openDataBase();
            Cursor c = db
                    .selectSQL("select credit_flag from SalesReturnHeader where upload = 'X' and RefModuleTId =" + StringUtils.getStringQueryParam(orderId) + " and RetailerID="
                            + StringUtils.getStringQueryParam(businessModel.getRetailerMasterBO().getRetailerID()));
            if (c.getCount() > 0) {
                while (c.moveToNext()) {
                    flag = c.getInt(0);
    ...
    private boolean isValueReturned(Context mContext, String orderId) {
        ...
        try {
            ...
            db.openDataBase();
            Cursor c = db
                    .selectSQL("select invoicecreated from SalesReturnHeader where upload ='X' and RefModuleTId =" + StringUtils.getStringQueryParam(orderId) + " and Retailerid="
...
//                                    updateExcessSih = headProductMasterBO.getDSIH() - prodQty;
    ...
    Vector<ProductMasterBO> preparePrintData(Context context, String orderId) {
        ...
        try {
            ...
            String invoiceno = "";
            Cursor cursor = db.selectSQL("select invoiceno,discount from invoicemaster where orderid =" + StringUtils.getStringQueryParam(orderId));
            ...
            String sb = "select PM.pid,PM.psname,ID.pcsQty,ID.caseQty,ID.OuterQty,ifnull(BM.Batchnum,\"\"),PM.duomQty,PM.douomQty,ID.Qty,PM.piece_uomid,PM.dUomId,PM.dOuomid,rate,caseprice,outerprice from InvoiceDetails ID " +
                    ...
                    "where ID.invoiceID=" + StringUtils.getStringQueryParam(invoiceno);
            Cursor c = db.selectSQL(sb);
            if (c.getCount() > 0) {
                ProductMasterBO productBO;
                while (c.moveToNext()) {
            ...
            String replQry = "select pid,batchid,uomid,qty from SalesReturnHeader SH" +
                    " inner join SalesReturnReplacementDetails SRPD on SRPD.uid = SH.uid where SH.invoiceid =" + StringUtils.getStringQueryParam(invoiceno);
    ...
    ArrayList<SchemeProductBO> downloadSchemeFreePrint(Context context, String orderId) {
        ...
        try {
            ...
            String invoiceId = "";
            Cursor invoice = db.selectSQL("Select invoiceno from invoicemaster where orderid=" + StringUtils.getStringQueryParam(orderId));
                    ...
                    if (businessModel.configurationMasterHelper.SHOW_UPDATE_SIH) {
                        if ("SRS".equals(bo.getReasonCategory())) {
                            ...
                            if (c != null && c.getCount() > 0) {
                                ...
                                String sihMastervalues = StringUtils.getStringQueryParam(product.getProductID())
                                        + ","
                                        + salRetSih + "," + batchid;
    ...
    public void updateRejectedOrder(Context context,String orderId,String referenceId){
        ...
        try {
            ...
            String values = StringUtils.getStringQueryParam(orderId) + "," + StringUtils.getStringQueryParam(referenceId)+", 'R'";
            db.insertSQL(orderDeliveryStatus, orderDeliveryStatus_cols, values);
