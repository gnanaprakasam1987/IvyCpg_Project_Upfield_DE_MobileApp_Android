...
public class OrderHelper {
    ...
    public void saveNearExpiry(Context context) {
        ...
        try {
            ...
            tid = businessModel.userMasterHelper.getUserMasterBO().getUserid() + ""
                    + businessModel.getRetailerMasterBO().getRetailerID() + ""
                    + SDUtil.now(SDUtil.DATE_TIME_ID);
            ...
            isData = false;
            for (ProductMasterBO skubo : businessModel.productHelper.getProductMaster()) {
                ...
                for (int j = 0; j < skubo.getLocations().size(); j++) {
                    ...
                    if (!skubo.getLocations().get(j).getNearexpiryDate().get(0)
                        ...
                        values = AppUtils.QT(tid)
                                ...
                                + AppUtils.QT(changeMonthNameToNoyyyymmdd(df.format(c
                                .getTime()))) + ","
                                + businessModel.getRetailerMasterBO().getRetailerID() + "," + skubo.getOwn();
                        db.insertSQL("NearExpiry_Tracking_Detail",
                                detailColumns, values);
                        isData = true;
                    }
            ...
            if (businessModel.productHelper.getTaggedProducts() != null) {
                for (ProductMasterBO skubo : businessModel.productHelper.getTaggedProducts()) {
                    if (skubo.getOwn() == 0) {
                        for (int j = 0; j < skubo.getLocations().size(); j++) {
            ...
            if (isData) {
                values = AppUtils.QT(tid) + "," + businessModel.getRetailerMasterBO().getRetailerID()
                        + "," + AppUtils.QT(SDUtil.now(SDUtil.DATE_GLOBAL)) + ","
    ...
    public void saveClosingStock(Context context,boolean isFromOrder) {
        try {
            ...
            boolean isData;
            String id = AppUtils.QT(businessModel.userMasterHelper.getUserMasterBO().getUserid()
                    + SDUtil.now(SDUtil.DATE_TIME_ID));
            if (businessModel.isEditStockCheck()) {
                Cursor closingStockCursor = db
                        .selectSQL("select StockID from ClosingStockHeader where RetailerID="
                                + businessModel.getRetailerMasterBO().getRetailerID() + "");
            ...
            if (businessModel.PRD_FOR_SKT) // Update is Productive only when the config is enabled.
                updateIsStockCheck(context);


            //Weightage Calculation
            if (businessModel.configurationMasterHelper.IS_FITSCORE_NEEDED) {
                FitScoreHelper.getInstance(context).getWeightage(businessModel.getRetailerMasterBO().getRetailerID(), DataMembers.FIT_STOCK);
            }
            ...
            if (isFromOrder)
                siz = businessModel.productHelper.getProductMaster().size();
            else
                siz = businessModel.productHelper.getTaggedProducts().size();

            for (int i = 0; i < siz; ++i) {
                if (isFromOrder)
                    product = businessModel.productHelper.getProductMaster().elementAt(i);
                else
                    product = businessModel.productHelper.getTaggedProducts().elementAt(i);
                ...
                for (int j = 0; j < siz1; j++) {
                    if ((businessModel.configurationMasterHelper.SHOW_STOCK_SP && product.getLocations().get(j).getShelfPiece() > -1)
                            || (businessModel.configurationMasterHelper.SHOW_STOCK_SC && product.getLocations().get(j).getShelfCase() > -1)
                            || (businessModel.configurationMasterHelper.SHOW_SHELF_OUTER && product.getLocations().get(j).getShelfOuter() > -1)
                            || (businessModel.configurationMasterHelper.SHOW_STOCK_CB && product.getLocations().get(j).getAvailability() > -1)
                        ...
                        int rField3 = 0;
                        if (businessModel.configurationMasterHelper.SHOW_STOCK_AVGDAYS) {
            ...
            if (isData) {
                ...
                values = (id) + ", " + AppUtils.QT(SDUtil.now(SDUtil.DATE_GLOBAL))
                        + ", " + AppUtils.QT(businessModel.getRetailerMasterBO().getRetailerID()) + ", "
                        + AppUtils.QT(businessModel.getRetailerMasterBO().getRetailerCode()) + ","
                        + AppUtils.QT(businessModel.getStockCheckRemark()) + "," + businessModel.getRetailerMasterBO().getDistributorId();

                if (businessModel.configurationMasterHelper.IS_ENABLE_SHARE_PERCENTAGE_STOCK_CHECK) {
                    String availabilityShare = (businessModel.getAvailablilityShare() == null ||
                            businessModel.getAvailablilityShare().trim().length() == 0) ? "0.0" : businessModel.getAvailablilityShare();
    ...
    private void updateIsStockCheck(Context context) {
        try {
            ...
            db.updateSQL("Update RetailerBeatMapping set isProductive='Y' where RetailerID ="
                    + businessModel.getRetailerMasterBO().getRetailerID() + " and BeatID=" + businessModel.getRetailerMasterBO().getBeatID());

            db.closeDB();

            // update loaded retailerMaster flag.
            int siz = businessModel.getRetailerMaster().size();
            for (int i = 0; i < siz; i++) {
                RetailerMasterBO ret = businessModel.retailerMaster.get(i);
            ...
            businessModel.getRetailerMasterBO().setProductive("Y");
    ...
    public void calculateFitscoreandInsert(DBUtil db, double sum, String module,Context context) {
        String headerID = "";
        ...
        try {
            Cursor closingStockCursor = db
                    .selectSQL("select Tid from RetailerScoreHeader where RetailerID=" + businessModel.getRetailerMasterBO().getRetailerID() + " and Date = " + AppUtils.QT(SDUtil.now(SDUtil.DATE_GLOBAL)));
            ...
            String tid = (headerID.trim().length() == 0) ? AppUtils.QT(businessModel.userMasterHelper.getUserMasterBO().getUserid() + SDUtil.now(SDUtil.DATE_TIME_ID)) : headerID;
            ...
            if (headerID.trim().length() == 0) {
                String retailerID = businessModel.getRetailerMasterBO().getRetailerID();
                ...
                Cursor achievedCursor = db
                        .selectSQL("select sum(0+ifnull(B.Score,0)) from RetailerScoreHeader A inner join RetailerScoreDetails B on A.Tid = B.Tid where A.RetailerID="
                                + businessModel.getRetailerMasterBO().getRetailerID() + " and A.Date = " + AppUtils.QT(SDUtil.now(SDUtil.DATE_GLOBAL)));
