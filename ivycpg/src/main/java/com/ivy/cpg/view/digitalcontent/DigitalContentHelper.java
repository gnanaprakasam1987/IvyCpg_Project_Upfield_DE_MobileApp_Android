...
// hii
            sb.append("SELECT " +
                    ...
                    "                      INNER JOIN StandardListMaster on ListId= mappingtype WHERE ListCode='LOCATION' and listtype='Digital_CRITERIA_TYPE')  LS ON DigitalContent.imgid=Ls.imgid and DigitalContent.GroupId=LS.GroupId " +
                    " " +
                    "LEFT JOIN (SELECT imgid,GroupId,mappingid ChannelId FROM DigitalContentMapping " +
                    " INNER JOIN StandardListMaster on ListId= mappingtype WHERE ListCode in ('CHANNEL','SUBCHANNEL') and listtype='Digital_CRITERIA_TYPE') CS ON  DigitalContent.imgid=CS.imgid and DigitalContent.GroupId=CS.GroupId " +
                    " " +
                    "LEFT JOIN (SELECT imgid,GroupId,mappingid AttributeID FROM DigitalContentMapping " +
                    "           INNER JOIN StandardListMaster on ListId= mappingtype WHERE ListCode='RTR_ATTRIBUTES' and listtype='Digital_CRITERIA_TYPE') AT ON  DigitalContent.imgid=AT.imgid and DigitalContent.GroupId=AT.GroupId " +
                    " " +
                    " LEFT JOIN (SELECT imgid,GroupId,mappingid PriorityBiD FROM DigitalContentMapping " +
                    "               INNER JOIN StandardListMaster on ListId= mappingtype WHERE ListCode='PRIORITY_PRD' and  listtype='Digital_CRITERIA_TYPE') PR ON  DigitalContent.imgid=PR.imgid and DigitalContent.GroupId=PR.GroupId " +
                    " " +
                    " LEFT JOIN (SELECT imgid,GroupId,mappingid RetailerID FROM DigitalContentMapping " +
                    "                          INNER JOIN StandardListMaster on ListId=mappingtype WHERE ListCode='RETAILER') RTR ON  DigitalContent.imgid=RTR.imgid and DigitalContent.GroupId=RTR.GroupId " +
                    " " +
                    "LEFT JOIN (SELECT imgid,GroupId,mappingid AccountID FROM DigitalContentMapping " +
                    "INNER JOIN StandardListMaster on ListId=mappingtype WHERE ListCode='ACCOUNT') ACC ON  DigitalContent.imgid=ACC.imgid and DigitalContent.GroupId=ACC.GroupId where ifNull(locationid,0) in(0,301,230,229,228,227,226,224,301) And ifnull(channelid,0) in (0,432,430,428,432) And ifnull(PriorityBiD,0) in (0,0) And ifnull(RetailerID,0) in (0,375) And ifnull(AccountID,0) in (0,0)");
