...
public class PrintLanguage{
    ...
    public static ePrintLanguages getLanguage(String sFileName){
        ...
        if(sFile.startsWith("csim")){
            ...
            sShortName="unknown";
            Commons.printException(TAG+",unknown print language: '"+sFile+"'");
        }
        ...
        for(int i=0; i<sFileName.length(); i++){
            ...
            if(code>=48 && code<=57){   //numbers are from 48 to 57
                try {
                    ...
                    Commons.printException(TAG+ ",exception converting '"+ sFileName.substring(i,1) +"' to Integer, using 2\"");
                    iPrintWidth=2;
                }
            }
        }}catch (Exception e){
            Commons.printException(TAG+ ",Exception in getPrintWidth: "+e);
        }
        return  iPrintWidth;
    }
