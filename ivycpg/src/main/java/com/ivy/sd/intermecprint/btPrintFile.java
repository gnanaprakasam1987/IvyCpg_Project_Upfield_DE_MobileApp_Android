...
public class btPrintFile {
	...
	public class MyRunnable implements Runnable {
		...
		/*
		 * // usage Thread t = new Thread(new MyRunnable(parameter)); t.start();
		 */
	}
	...
	private static final UUID UUID_SPP = UUID
	...
	public static final int STATE_CONNECTED = 3; // now connected to a remote
													// device
	public static final int STATE_DISCONNECTED = 4; // now connected to a remote
													// device
	...
	private synchronized void setState(int state) {
		if (D)
			Commons.print(TAG+ ",setState() " + mState + " -> " + state);
		mState = state;

		addText(msgTypes.STATE, state);
		// Give the new state to the Handler so the UI Activity can update
		// Message msg = new Message();//
		// mHandler.obtainMessage(_Activity.MESSAGE_DEVICE_NAME);
	...
	public synchronized void connect(BluetoothDevice device) {
		if (D)
			Commons.print(TAG+ ",connect to: " + device);
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			try {
				...
				Commons.printException(TAG+ ",create() failed", e);
			}
			mmSocket = tmp;
		}

		public void run() {
			Commons.print(TAG+ ",BEGIN mConnectThread");
	...
	private class ConnectedThread extends Thread {
		...
		public ConnectedThread(BluetoothSocket socket) {
			Commons.print(TAG+ ",create ConnectedThread");
			mmSocket = socket;
		...
		public void run() {
			Commons.print(TAG+ ",BEGIN mConnectedThread");
			...
			while (true) {
				try {
					...
					mHandler.obtainMessage(msgTypes.MESSAGE_READ, bytes, -1,
							buffer).sendToTarget();
				} catch (IOException e) {
					Commons.printException(TAG+ ",disconnected", e);
					connectionLost();
					break;
				}
			}
		...
		public void write(byte[] buffer) {
			...
			try {
				...
				Commons.printException(TAG+ ",Exception during write", e);
			}
			addText("write done");
		}
	...
	void addText(String msgType, int state) {
		...
		// msg = mHandler.obtainMessage(msgTypes.MESSAGE_STATE_CHANGE);//
		// mHandler.obtainMessage(_Activity.MESSAGE_DEVICE_NAME);
		// mHandler.obtainMessage(BluetoothChat.MESSAGE_STATE_CHANGE, state,
		// -1).sendToTarget();
		bundle.putInt(msgType, state);
		msg.setData(bundle);
		msg.arg1 = state; // we can use arg1 or the bundle to provide additional
							// information to the message handler
		mHandler.sendMessage(msg);
		Commons.print(TAG+ ",addText: " + msgType + ", state=" + state);
	}

	void log(String msg) {
		if (D)
			Commons.print(TAG+","+ msg);
	}
