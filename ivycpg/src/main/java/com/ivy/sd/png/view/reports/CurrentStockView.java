...
public class CurrentStockView extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.fragment_stock_report, container,
				false);
		...
		Spinner spinnerbrand = (Spinner) view.findViewById(R.id.brandSpinner);
		...
		mylist = bmodel.reportHelper.downloadCurrentStockReport();
		...
		spinnerbrand.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				updateStockReportGrid(temp.getProductid());
				productName.setText("");
			}
	...
	private void updateStockReportGrid(int brandId) {
		...
		int siz = mylist.size();
		ArrayList<StockReportBO> temp = new ArrayList<StockReportBO>();
		for (int i = 0; i < siz; ++i) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (bmodel.configurationMasterHelper.IS_EOD_STOCK_SPLIT) {
				...
				if (product.isBaseUomCaseWise()&&product.getCaseSize()!=0) {
					...
					holder.sihCase.setText(totalQty / product.getCaseSize()+"");
					rem_sih=totalQty % product.getCaseSize();
				}
				if(product.isBaseUomOuterWise()&&product.getOuterSize()!=0) {
					if (isUomWiseSplitted) {
						holder.sihOuter.setText(rem_sih / product.getOuterSize()+"");
						rem_sih=rem_sih % product.getOuterSize();
					}
					else{
						isUomWiseSplitted = true;
						holder.sihOuter.setText(totalQty / product.getOuterSize()+"");
