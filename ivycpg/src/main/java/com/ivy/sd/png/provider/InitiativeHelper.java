...
public class InitiativeHelper {
	 ...
	 * retailer.
	 * 
	 * @param isUpdateAcheivement
	 *            - if true the Update the status in InitiativeCoverageReport.
	 * @param retailerId
	 * @param forCoverageReport
	 *            - if true then productmaster will not be consider for
	 *            calculation.
	 */
	...
	public Vector<InitiativeReportBO> downloadInitReport() {
		...
		try {
			...
			Cursor c = db
					.selectSQL("select distinct(B.RetailerId),B.RetailerName,B.isDeviated," +
							...
							+ " where  (RMI.isToday=1 or B.isdeviated='Y')  order by seq");
			if (c != null) {
				while (c.moveToNext()) {
