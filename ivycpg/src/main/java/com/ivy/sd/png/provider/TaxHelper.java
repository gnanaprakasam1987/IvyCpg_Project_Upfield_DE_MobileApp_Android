...
public class TaxHelper {
    ...
    public void downloadProductTaxDetails() {
        ...
        try {
            ...
            if (!mBusinessModel.configurationMasterHelper.IS_GST && mBusinessModel.configurationMasterHelper.IS_LOCATION_WISE_TAX_APPLIED) {
            ...
            } else if (mBusinessModel.configurationMasterHelper.IS_LOCATION_WISE_TAX_APPLIED) {
    ...
    public void insertProductTaxList(String invoiceid, DBUtil db) {
        ...
        if (mTaxProdList != null) {//
            ...
            for (TaxBO taxBO : mTaxProdList) {
                for (ProductMasterBO sku : mBusinessModel.productHelper.getProductMaster()) {
                    ...
                    for (int i = 0; i < taxSize; i++) {
                        if (sku.getProductID().equals(taxBO.getPid() + "")
                            ...
                            if (sku.getOrderedCaseQty() > 0
                                ...
                                sb = new StringBuffer();
                                sb.append(mBusinessModel.QT(mBusinessModel
                                        .getRetailerMasterBO().getRetailerID())
                                        + ",");
                                sb.append(mBusinessModel.QT(invoiceid) + ","
                                        + sku.getProductID() + ","
