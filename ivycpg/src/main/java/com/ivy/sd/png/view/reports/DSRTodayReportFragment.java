...
public class DSRTodayReportFragment extends Fragment {
		...
		try {
			...
			Cursor c = db
					.selectSQL("select  (select count(o.retailerid)  from OrderHeader o inner join RetailerMaster r on o.retailerid=r.retailerid "
							...
							+ " where  ((RMI.istoday=1 and r.isDeadStore = 'Y') or (r.isdeviated='Y' and r.isDeadStore = 'Y')))"
							+ ",(select count(R.retailerid) from Retailermaster R"
							+ " inner join Retailermasterinfo RMI on Rmi.retailerid= R.retailerid"
							+ " where ((RMI.istoday=1 and isDeadStore = 'N') or (isdeviated='Y'  and isDeadStore = 'N')))");
	...
	private void downloadMTDGap() {
		try {
			...
			Cursor c2 = db
					.selectSQL("select  distinct RM.RetailerID   from RetailerMaster RM"
							+  " inner join Retailermasterinfo RMI on Rmi.retailerid= RM.retailerid "
							+ " where   (isdeviated='Y' or RMI.isToday='1' and isDeadStore = 'N' )");
			if (c2 != null) {
				while (c2.moveToNext()) {
	...
	public int getTotalLinesTargetForToday() {
		try {
			...
			db.openDataBase();
			Cursor c = db
					.selectSQL("select sum(Lines) from DTPMaster where retailerID in (select RM.RetailerID from RetailerMaster RM inner join Retailermasterinfo RMI on Rmi.retailerid= RM.retailerid where RMI.isToday=1 or isDeviated='Y')");
			if (c.getCount() > 0) {
				while (c.moveToNext()) {
